...
describe('[eth/counterfactual] buildCreate2Address', () => {
  it('create counterfactual address from contract', () => {
    ...
    const bytecodefull = `${bytecode}${iden3.counterfactual.encodeParam('address', kop).slice(2)}${iden3.counterfactual.encodeParam('address', relayAddr).slice(2)}${iden3.counterfactual.encodeParam('address', krec).slice(2)}${iden3.counterfactual.encodeParam('address', krev).slice(2)}${iden3.counterfactual.encodeParam('address', iden3implAddr).slice(2)}`;
