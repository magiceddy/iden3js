...
const calculateIDAddress = function calculateIDAddress(kop, krec, krev, relayAddr, iden3implAddr, iden3deployerAddr, bytecode) {
  const bytecodefull = `${bytecode}${encodeParam('address', kop).slice(2)}${encodeParam('address', relayAddr).slice(2)}${encodeParam('address', krec).slice(2)}${encodeParam('address', krev).slice(2)}${encodeParam('address', iden3implAddr).slice(2)}`;
  const salt = 0;
