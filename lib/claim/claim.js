...
class GenericClaim {
  ...
  bytes() {
    ...
    b = Buffer.concat([b, utils.uint32ToEthBytes(this.claim.baseIndex.indexLength)]);
    b = Buffer.concat([b, utils.uint32ToEthBytes(this.claim.baseIndex.version)]);
...
class AuthorizeKSignClaim {
  ...
  bytes() {
    ...
    b = Buffer.concat([b, utils.uint32ToEthBytes(this.claim.baseIndex.indexLength)]);
    b = Buffer.concat([b, utils.uint32ToEthBytes(this.claim.baseIndex.version)]);
    b = Buffer.concat([b, utils.hexToBytes(this.claim.extraIndex.keyToAuthorize)]);
