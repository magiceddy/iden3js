...
describe('[sparse-merkle-tree] Add claims in different orders into six different trees', () => {
  it('add claims in differnet orders into six trees', () => {
    ...
    const mt1 = new iden3.sparseMerkleTree.SparseMerkleTree(db, idAddr);
    const mt2 = new iden3.sparseMerkleTree.SparseMerkleTree(db, idAddr);
    const mt3 = new iden3.sparseMerkleTree.SparseMerkleTree(db, idAddr);
    const mt4 = new iden3.sparseMerkleTree.SparseMerkleTree(db, idAddr);
    const mt5 = new iden3.sparseMerkleTree.SparseMerkleTree(db, idAddr);
    const mt6 = new iden3.sparseMerkleTree.SparseMerkleTree(db, idAddr);
...
describe('[sparse-merkle-tree] Generate proof', () => {
  ...
  it('with 4 claims', () => {
    ...
    expect(iden3.utils.bytesToHex(proof)).to.be.equal('0x0001000000000000000000000000000000000000000000000000000000000001' + '05086d2d031b3aeb91b850c7a0280499ded7ba4b8b25caffff5dc754ed207eb8');
  });
  it('with 64 claims', () => {
    const mt = new iden3.sparseMerkleTree.SparseMerkleTree(db, idAddr);
    for (let i = 0; i < 64; i++) {
    ...
    const proof = mt.generateProof(proofClaim.slice(2));
    expect(iden3.utils.bytesToHex(proof)).to.be.equal('0x000700000000000000000000000000000000000000000000000000000000005f' + '1a412100b0a427796bf11048f3759c9c2163dc39f44d2b243dd8d8fee531800d' + '21284e40a8d3e9658d429ebedbef7d509de684b9143aac0b1be3c3979a4e5ed0' + '005844e6e7d83169766472da339496a1663ebc14a1016dd39105ca13848f68d4' + '102e955ce94001fa069fdd144a3fda637020dd21f1a4a9f23ae0665a4ed27457' + '28b82cce3a8d858c295e42c8f058b4a73ca39b0222754165db9f8c7ecf5f431a' + '1e220bd9bea43d106d41b7bc2a5d2e88d0fc4f0429c3f8de7059a0bf93e40212');
  });
});
...
describe('[sparse-merkle-tree] Verify proof', () => {
  it('proof-of-existence', () => {
    ...
    expect(proofHex).to.be.equal('0x0003000000000000000000000000000000000000000000000000000000000007' + '2e2e61a54ec48cb031effbf00420cd06d707535616965f1ffda2edd1006b807c' + '0b9a1a9cc13e5fe12e380fb702c10fde1a9201b7f89e25051f57e00862f20522' + '00b1574ea5a96e97ff7b0c964c14d0ad7b9da5b56d068b3aabe10fd3051b0d2a');
  ...
  it('proof-of-non-existence non empty node value', () => {
    ...
    const rootHex = iden3.utils.bytesToHex(mt.root);
    const proofHex = iden3.utils.bytesToHex(proof);
    expect(proofHex).to.be.equal('0x030400000000000000000000000000000000000000000000000000000000000b' + '0a439bd423b069c01717cd8641d610f286ec5062c9ecee6f2412af76ff551cb5' + '1c0fd5c25407d0220a0bcbc6734908153fd18ec43ee62ee157030462c43f537d' + '2d5899d3a66630d86c3b9ff896a99b0f2b9e7afc49b69175ed07773ae39263f5' + '149648851923be5e707629f0619a1f391452d3c252291d5492d5f9280542380f' + '1541a6b5aa9bf7d9be3d5cb0bcc7cacbca26242016a0feebfc19c90f2224baed');
    const check = iden3.sparseMerkleTree.checkProof(rootHex, proofHex, hiHex, hvHex);
    expect(check).to.be.equal(true);
  });
  it('proof-of-non-existence empty node value', () => {
    ...
    const rootHex = iden3.utils.bytesToHex(mt.root);
    const proofHex = iden3.utils.bytesToHex(proof);
    expect(proofHex).to.be.equal('0x0105000000000000000000000000000000000000000000000000000000000017' + '2e2e61a54ec48cb031effbf00420cd06d707535616965f1ffda2edd1006b807c' + '0b9a1a9cc13e5fe12e380fb702c10fde1a9201b7f89e25051f57e00862f20522' + '1e1b8f66c3bd26be093e358ed6c54f9d1986411ea404d01482fcf26b04912a0c' + '17974735b062a464506127e92858392e853b6abbcb3a1d93a5924af42198c3d1');
    const check = iden3.sparseMerkleTree.checkProof(rootHex, proofHex, hiHex, hvHex);
    expect(check).to.be.equal(true);
  });
});
