...
class Dapp {
  ...
  async recv_wallet(message) {
    ...
    if (op === 'call') {
      ...
      // build signature
      const fwdsigpre = `0x${Buffer.concat([buf(uint8(0x19)), buf(uint8(0)), buf(idaddr), buf(uint256(fwdnonce)), buf(args.to), buf(args.data), buf(uint256(args.value)), buf(uint256(args.gas))]).toString('hex')}`;
      const fwdsig = ethutil.ecsign(buf(sha3(fwdsigpre)), ksignpvk);

      args.fwdsig = `0x${Buffer.concat([fwdsig.r, fwdsig.s, buf(fwdsig.v)]).toString('hex')}`;
